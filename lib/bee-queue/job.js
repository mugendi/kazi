...
Job.prototype.setId = function (jobId) {
  var self = this;
  //set jobId
...
Job.prototype.now = function (jobId) {
  var self = this;
  this.immediate = jobId;
  return this;
};
...
Job.prototype.save = function (cb) {
  ...
  // console.log(this.queue.toKey('id'));

  this.queue.client.evalsha(lua.shas.addJob, 5,
    this.queue.toKey('id'), this.queue.toKey('jobs'), this.queue.toKey('waiting'), (this.jobId || false), (this.immediate || false), this.toData(),
    function (err, jobId) {
      /* istanbul ignore if */
      if (err) return cb(err);
      self.id = jobId;
