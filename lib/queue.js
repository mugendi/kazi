	...
	setInterval(function(){
		...
		if(autoclearFinishedJobs){
			...
			redisClient.zremrangebyscore(setCmd,function(err,res){
				...
				if(res){
					//emit job cleared
					server.emit('jobsCleared',res+' job(s) older than '+(autoclearFinishedJobsAfter)+' seconds successfully cleared!');
				}
		...
		if(!/win/g.test(os.platform())){
			...
			exec(cmd , {silent:true}, function(code, output) {
				...
				output.split(/[\r\n]/g).forEach(function(line){
					if(line.trim().length){
						vals=line.split(':');
						info[vals[0].replace('_human','')]=vals[1];					
					}					
				});
			  
			  	server.emit('memoryInfo','Redis memory consumption', {},{},info)
