...
var autoclearFinishedJobsAfter=2*3600 ;//2 hours;
var rescheduleStuckJobsAfter=60*5; //5 mins (always in seconds)
var autoclearFinishedJobs=true;
var logger=require('./log.js')

var config=require(path.join('..','data','config.json'));
var saved_jobs=path.join(__dirname,'..','data','jobs.json');
...
Queue.prototype.queueJob= function (jobs,update,callback,client){
	...
	if(!_.isArray(jobs) && !_.isObject(jobs)){
		//wrong job
		//emit
		server.emit('jobType','Wrong job Type. Jobs can only be submitted as arrays or objects.',client, job,server_info);
	}
		...
		server.uniqueJob(job,function(err,unique){
			...
			if(unique || update){
				...
				redisClient.hset(jobHashKey,job.id,JSON.stringify(job),function(err,res){
					...
					redisClient.zadd(jobOrderKey,priority,job.id,function(err,res){
						...
						if(ttl>0){
							...
							server.killJob(job.id,ttl,function(err,dieAt){
								//remove ttl
								server.emit('jobDeath','Job [JOB:ID '+job.id+'] will die at '+dieAt+"",client,job,server_info);
							})
						}
					...
					server.emit('job'+evt,'Job '+(evt.toLowerCase())+( (client) ? ' by Client ('+client.name+')':'')+ (delay?'. A delay of '+delay+' seconds has also been effected!':'') + '.', client, job,server_info);
	...
	redisClient.zadd(jobCompleteSetKey,stateTime,JSON.stringify(job),function(err,res){
		...
		redisClient.zrem(jobOrderKey,job.id,function(err,res){
			...
			redisClient.hdel(jobHashKey,job.id,function(err,res){
				...
				//emit job completion
				server.emit('jobFinished','Job finished by Client ('+client.name+').',client, job,server_info);
			});
	...
	server.finishJob(client,job,function(){
		...
		redisClient.zrangebyscore(jobDeathOrder,0,now,function(err,jobIds){
			...
			if(_.indexOf(jobIds,job.id)>-1){
				//emit event
				server.emit('jobDeath','Job [JOB:ID '+job.id+'] has died after ttl (Time to live) has elapsed!',client, job,server_info);
	...
	server.uniqueJob(job_id,function(err,unique){
		...
		if(!unique){
			...
			redisClient.zadd(jobDeathOrder,dieAt,job_id,function(err,res){
				...
				server.emit('jobDeath','Job [JOB:ID '+job_id+'] has been scheduled to die at '+dieAtTime+' after TTL (Time to live) of '+TTL+' seconds!',{},{},server_info);
...
Queue.prototype.freeClient=function(client,callback){
	...
	redisClient.hdel(busyClientsHashKey,client.name,function(err,res){
		//emit event
		server.emit('clientFreed','Client ('+client.name+') freed. Client can now request new jobs!', client,{},server_info);
...
Queue.prototype.requestJob = function(client,callback,jobIds){
	...
	server.emit('clientJobRequest','Client ('+client.name+') has requested for a new job.', client, {},server_info);
			...
			if(jobId){
				...
				server.isBusyClient(client,function(err,isBusy){
					...
					if(isBusy){
						...
						//emit 
						server.emit('clientBusy','Client ('+client.name+') is busy! Cant serve new job!', client, {},server_info);
				...
				server.emit('clientNotServed','Client ('+client.name+') NOT served! There are no jobs to serve!', client, {},server_info);
			}
			...
			redisClient.hset(busyClientsHashKey,client.name,job.id,function(err,res){
				...
				redisClient.zadd(jobOrderKey,(now*-1),job.id,function(err,res){
					...
					redisClient.hset(jobHashKey,job.id,JSON.stringify(job),function(err,res){
						...
						//emit job served
						server.emit('clientServed','Client ('+client.name+') served.', client, job,server_info);
			...
			if(!res){
				...
				redisClient.set(clientKey, JSON.stringify(clientObj) ,function(err,res){
					...
					server.expireClient(clientObj,function(err,client){
						...
						//emit client liste
						server.emit('clientListed','Client ('+client.name+') listed and expires in '+server.options.expireClientsAfter+' seconds.', clientObj,{},server_info);	
				...
				//emit client liste
				server.emit('clientExists','Client ('+client.name+') already listed!', clientObj,{},server_info);
			}
	...
	setInterval(function(){
		...
		if(autoclearFinishedJobs){
			...
			var time_to_expire=(moment().subtract(autoclearFinishedJobsAfter,'seconds').unix()), // (autoclearFinishedJobsAfter) microseconds ago
				...
				setCmd=[jobCompleteSetKey,start_of_time,time_to_expire]; //delete any keys between 1970 and (autoclearFinishedJob
			...
			redisClient.zremrangebyscore(setCmd,function(err,res){
				...
				if(res){
					//emit job cleared
					server.emit('jobsCleared',res+' job(s) older than '+(autoclearFinishedJobsAfter)+' seconds successfully cleared!',{},{},server_info);
				}
	 		
	 		});		
		}
		
		//RESCHEDULE STUCK JOBS
				...
				jobIds.forEach(function(jobId){
					...
					redisClient.hget(jobHashKey,jobId,function(err,res){
						if(res){
							...
							server.emit('jobRescheduled','Job ID:'+job.id+' has been successfully rescheduled after being in active for '+rescheduleStuckJobsAfter+' seconds', {},job,server_info);
		...
		//SERVER REDIS MEMORY INFO
		server_info={
			server_started:server_start.fromNow()
		};

		if(!/win/g.test(os.platform())){

			var cmd = 'redis-cli info | grep memory.*human;'
			// console.log(cmd);
			...
			exec(cmd , {silent:true}, function(code, output) {
				...
				//make server_info object
				output.split(/[\r\n]/g).forEach(function(line){
					if(line.trim().length){
						vals=line.split(':');
						server_info[vals[0].replace('_human','')]=vals[1];					
					}					
				});
			  
			  	server.emit('memoryInfo','Redis memory consumption', {},{},server_info)
		...
		//BACKUP CURRENT FOR RESTART
		console.log(saved_jobs)

		//get all jobs
		redisClient.hgetall(jobHashKey,function(err,res){
			if(res){
				...
				// console.log(JSON.stringify(jobs,0,4))

				jsonfile.writeFile(saved_jobs, jobs, function (err) {
				  // console.error(err)
				});
			}
