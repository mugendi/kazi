	...
	jobs.forEach(function(job,i){
		...
		server.uniqueJob(job,function(err,unique){
			...
			if(unique || update){
				...
				job.roundTime=(job.updated)? 
					(nowUnix - (moment(job.updated, moment.ISO_8601).unix()*1000) ):0;
				job.timeAlive=(job.scheduled)? 
					(nowUnix - (moment(job.scheduled, moment.ISO_8601).unix()*1000) ) : 0;
					
				job.updated= nowISO;
				job.scheduled= job.scheduled || nowISO;
