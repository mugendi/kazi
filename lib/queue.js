...
var server={},
	Queue = function (options) {
		...
		if(_.size(server.options.logger)){
			// console.log(logger)
			var log = new logger(server,server.options.logger,process.env.TZ);
		}
...
Queue.prototype.queueJob= function (jobs,update,callback,client){
	...
	jobs.forEach(function(job,i){
		...
		if(server.options.strictFIFO || !_.isUndefined(job.delay) ){
			...
			job.priority=(job.priority.toLowerCase()=='high') ? 0 : moment().add(i,'seconds').unix();
