...
logger.save={
	...
	firebase:function(obj,log_opts){
		...
		for(var i in obj){
			console.log(obj[i],truely(obj[i]));
...
logger.log=function(server,log_opts){
	...
	log_events.forEach(function(evt){
		...
		server.on(evt, function(msg,client,job,info){
			...
			if(client){
				var obj={
					...
					job:_.merge(
						...
						{
							//pick only first 20 keys of any job
							data: _.pick( job.data, _.first(_.keys(job.data),20)) 
						}
					) || {},
					msg:msg,
					info:info
