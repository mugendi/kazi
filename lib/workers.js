...
domain.on('error', function(error) {
	...
	*/
	if(typeof error.stack !=='undefined' && error.stack!=='undefined'){
		logger.msg_log({
...
function requestJob(){
	...
	domain.log('['+now()+'] '+client.name +': Requesting new job...');
	...
	request.post(post, function(err,httpResponse,body){
		...
		if(body){
			...
			if(!_.isEmpty(job)){
				domain.log('['+now()+'] '+client.name +': Job allocated [JOB:'+job.id+']');
				...
				domain.log('['+now()+'] '+client.name +': ...waiting before next Job Request!');
	...
	if(job && !_.isUndefined(job.name)){
		...
		if( _.has(workers,job.name) && _.has(workers[job.name],'run') ){
			...
			domain.run(function() {	
				...
				workers[job.name].run(job,domain,function(result){
